...
import { parseNotesField, aggregateByValue, aggregateByPeriod } from '../utils/notes-parser.js';
...
**Ideal for:** Initial analysis, reporting, performance reviews`,
        ...
        try {
            ...
            if (params.top_opportunities_count > 0) {
                const topOpps = await client.searchRead('crm.lead', [...domain, ['active', '=', true], ['probability', '<', 100]], ['id', 'name', 'expected_revenue', 'probability', 'stage_id'], { limit: params.top_opportunities_count, order: 'expected_revenue desc' });
...
- Compare with won data for context (win/loss ratio)`,
        ...
        try {
            ...
            if (params.group_by === 'sector') {
                const bySector = await client.readGroup('crm.lead', domain, ['sector', 'expected_revenue:sum', 'id:count'], ['sector']);
                analysis.by_sector = bySector.map(s => ({
                    sector: s.sector || 'Not Specified',
                    count: s.id || 0,
                    percentage: totalLost > 0 ? (s.id / totalLost) * 100 : 0,
                    lost_revenue: s.expected_revenue || 0,
                    avg_deal: s.id > 0 ? (s.expected_revenue || 0) / s.id : 0
            ...
            if (params.group_by === 'specification') {
                const bySpec = await client.readGroup('crm.lead', domain, ['specification_id', 'expected_revenue:sum', 'id:count'], ['specification_id']);
                analysis.by_specification = bySpec.map(s => ({
                    specification_id: Array.isArray(s.specification_id) ? s.specification_id[0] : 0,
                    specification_name: Array.isArray(s.specification_id) ? s.specification_id[1] : 'Not Specified',
                    count: s.id || 0,
                    percentage: totalLost > 0 ? (s.id / totalLost) * 100 : 0,
                    lost_revenue: s.expected_revenue || 0,
                    avg_deal: s.id > 0 ? (s.expected_revenue || 0) / s.id : 0
            ...
            if (params.group_by === 'lead_source') {
                const byLeadSource = await client.readGroup('crm.lead', domain, ['lead_source_id', 'expected_revenue:sum', 'id:count'], ['lead_source_id']);
                analysis.by_lead_source = byLeadSource.map(s => ({
                    lead_source_id: Array.isArray(s.lead_source_id) ? s.lead_source_id[0] : 0,
                    lead_source_name: Array.isArray(s.lead_source_id) ? s.lead_source_id[1] : 'Not Specified',
                    count: s.id || 0,
                    percentage: totalLost > 0 ? (s.id / totalLost) * 100 : 0,
                    lost_revenue: s.expected_revenue || 0,
                    avg_deal: s.id > 0 ? (s.expected_revenue || 0) / s.id : 0
...
- Benchmarking against averages`,
        ...
        try {
            ...
            if (params.compare_type === 'periods') {
                ...
                const lostStats = await client.readGroup('crm.lead', [[groupField, '!=', false], ['active', '=', false], ['probability', '=', 0]], [groupField, 'id:count'], [groupField]);
                const entities = [];
...
- XLSX format is recommended for Excel users (smaller files)`,
        ...
        try {
            ...
            try {
                ...
                const { records, totalFetched, totalAvailable } = await client.searchReadPaginated(model, domain, fields, {
    ...
    // ============================================
    server.registerTool('odoo_crm_cache_status', {
        ...
        try {
            ...
            if (stats.size === 0) {
                ...
                for (const key of stats.keys) {
                    ...
                    if (key.startsWith('crm:stages'))
                        friendlyName = 'Pipeline Stages';
                    else if (key.startsWith('crm:lost_reasons'))
                        friendlyName = `Lost Reasons (${key.includes('true') ? 'including inactive' : 'active only'})`;
                    else if (key.startsWith('crm:teams'))
                        friendlyName = 'Sales Teams';
                    else if (key.startsWith('crm:salespeople'))
                        friendlyName = key.includes('team:') ? `Salespeople (team ${key.split(':').pop()})` : 'All Salespeople';
                    output += `- ${friendlyName}\n`;
                }
...
- Analyze revenue distribution by state`,
    ...
    // ============================================
    server.registerTool('odoo_crm_get_color_trends', {
...
- Detection rate statistics`,
    ...
    // ============================================
    server.registerTool('odoo_crm_search_rfq_by_color', {
...
- Notes excerpt showing color context`,
    ...
    // ============================================
    server.registerTool('odoo_crm_analyze_notes', {
...
- System types used: extract_field="Specified System", group_by="value"`,
        ...
        try {
            return await useClient(async (client) => {
                ...
                const leads = await client.searchRead('crm.lead', domain, ['id', 'name', 'description', 'expected_revenue', dateField], { limit: 5000, offset: 0, order: `${dateField} desc` });
                // Extract field values from notes
                const extractField = params.extract_field || 'Specified Colours';
                const extractedData = leads.map(lead => ({
                    lead_id: lead.id,
                    lead_name: lead.name,
                    extracted_value: parseNotesField(lead.description, extractField),
                    date_value: lead[dateField] || null,
