 ...
 */
export function aggregateByColor(leads: LeadWithColor[]): Map<string, LeadWithColor[]> {
  const grouped = new Map<string, LeadWithColor[]>();
 ...
 */
export function buildColorTrendsSummary(
  leads: LeadWithColor[],
  ...
  const periods: ColorTrendPeriod[] = [];
...
function calculateColorTrends(
  ...
  return colorDistribution.map(c => {
    ...
    if (firstCount === 0) {
      return { color_category: c.color_category, trend: 'up' as const, change_percent: 100 };
    }
 ...
 */
export function filterLeadsByColor(
  leads: LeadWithColor[],
  options: {
