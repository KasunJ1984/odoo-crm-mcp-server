 ...
 * // Returns: "Oyster Grey"
 */
export function parseNotesField(
  ...
  const regex = new RegExp(
    `${escapedPattern}\\s*[=:]\\s*(.+?)(?=\\s*Specified\\s+\\w+\\s*[=:]|$)`,
    'i'
  );
 ...
 */
export function parseAllNotesFields(
  description: string | null | undefined
...
export function aggregateByValue(
  ...
  const result: AggregatedValue[] = Array.from(grouped.entries())
    .map(([value, data]) => ({
      value,
      count: data.count,
...
export function aggregateByPeriod(
  ...
  const periodMap = new Map<string, LeadWithExtractedNotes[]>();
